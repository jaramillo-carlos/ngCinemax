...
describe('MoviesFilterComponent', () => {
  ...
  it('Should generate 8 date items requireds', () => {
    component.ngOnInit();
    for (let i = 0; i <= 7; i++) {
      const date: Date = new Date();
      date.setDate(date.getDate() + i);

      // expect(component.dates[i]).toEqual(date);
      expect(component.dates[i].toDateString()).toEqual(date.toDateString());
    }
  });

  it('should injected _landingService', () => {
    expect(service).toBeTruthy();
  });

  it('should call getMovies() from _landingService on ngOnInit', () => {
