...
describe('LandingPageService', () => {
  ...
  it('should call method getSocialNetworks() and return socialnetworks', () => {
    service.getSocialNetworks().subscribe(networks => {
      ...
      expect(networks).toEqual(mockResponseNetworks);
    });

    const req = httpMock.expectOne('https://ngcinemax-71540.firebaseio.com/RedesSociales.json');
    expect(req.request.method).toBe('GET');
    req.flush(mockResponseNetworks);
  });

  it('should call method getMovies() and return movies', () => {
    service.getMovies().subscribe( (movies: Movie[]) => {
      const billboards = movies.filter( (movie: Movie) => movie.cartelera );
      const premieres = movies.filter( (movie: Movie) => !movie.cartelera );

      expect(billboards.length).toBe(6);
      expect(billboards.length).toBeGreaterThanOrEqual(6);

      expect(premieres.length).toBe(6);
      expect(premieres.length).toBeGreaterThanOrEqual(6);

      expect(movies).toEqual(mockResponseMovies);
